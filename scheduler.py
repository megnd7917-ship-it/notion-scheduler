...
def calculate_bottleneck_task_ids(tasks, completed):
    """Return unfinished tasks that currently block other unfinished work."""
    tasks_by_id, dependents = build_dependency_graph(tasks)

    remaining = {
        task["page_id"]: max(
            0,
            task["minutes"] - completed.get(task["page_id"], 0),
        )
        for task in tasks
    ...
    active_ids = {
        task_id
        for task_id, minutes in remaining.items()
        if minutes > 0
    }

    bottlenecks = set()

    def has_active_dependent(task_id, seen=None):
        if seen is None:
            seen = set()
        if task_id in seen:
            return False
        ...
        for dependent_id in dependents.get(task_id, set()):
            if dependent_id not in active_ids:
                continue
            return True
    ...
    for task_id in active_ids:
        if has_active_dependent(task_id):
            bottlenecks.add(task_id)
...
def calculate_status(tasks, completed, focus_blocks, completed_pfs_minutes=0):
    ...
    for task in tasks:
        ...
        if task["deadline"]:
            ...
            if deadline <= now + timedelta(days=18):
                ...
                deadline_task_ids.add(task["page_id"])
    ...
    bottleneck_task_ids = calculate_bottleneck_task_ids(
        tasks,
        completed,
    )

    # A task contributes to the deficit only when the current schedule
    # actually has a shortfall. If there is enough capacity, there is no
    # deficit icon to assign. The task set is the same deadline-constrained
    # work used by the existing status calculation; this does not create a
    ...
    return {
        ...
        "bottleneck_task_ids": bottleneck_task_ids,
        "deficit_task_ids": deficit_task_ids,
    }
...
def actual_deadline_end(task):
    ...
    if not deadline:
        return None
    return deadline.get("end") or deadline.get("start")


def sync_task_allocation_icons(allocations, tasks, status_info):
    """Set each allocation's single Notion page icon.

    Precedence is intentionally simple and fixed:
        bottleneck > deficit > due today > due in 2-3 days > none

    Completed and held allocations remain visible, but receive no icon.
    """
    tasks_by_id = {task["page_id"]: task for task in tasks}
    bottleneck_ids = set(status_info.get("bottleneck_task_ids", set()))
    deficit_ids = set(status_info.get("deficit_task_ids", set()))
    now = datetime.now(TZ)

    for allocation in allocations:
        task_id = allocation["master_ids"][0] if allocation["master_ids"] else None
        task = tasks_by_id.get(task_id)

        icon = None

        if task and not allocation["completed"] and not allocation.get("hold", False):
            if task_id in bottleneck_ids:
                icon = {"type": "emoji", "emoji": "⚠️"}
            elif task_id in deficit_ids:
                icon = {"type": "emoji", "emoji": "☢️"}
            else:
...
def rebuild(tasks, all_focus_blocks, focus_blocks, allocations):
    ...
    # Refresh allocations after creation so the same existing icon logic
    # can update both retained historical allocations and newly created
    # pages. No separate icon/scheduling system is introduced.
    current_allocations = read_allocations()
    sync_task_allocation_icons(
        current_allocations,
        tasks,
        status_info,
    )
...
def main():
    ...
    if not force_rebuild and not changed and not reconsider_requested:
        # Icon status depends on the current time as well as task/dependency
        # state, so it must be refreshed even when no schedule rebuild is
        # necessary. This keeps the existing allocations' icons accurate
        # without creating or deleting any allocations.
        sync_task_allocation_icons(
            allocations,
            tasks,
            schedule_status["status_info"],
        )
        print("No relevant changes detected.")
        print("Task Allocation icons refreshed; Daily Plan unchanged.")
        return
